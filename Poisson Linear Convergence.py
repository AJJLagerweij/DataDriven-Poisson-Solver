...
from problem import Homogeneous, Hat
...
if __name__ == "__main__":
    ...
    # rhs = partial(rhs_hats, [(0.4, 0.6,  0.00)])  # rhs in test setup.
    ...
    plt.loglog(intermediate_J1Gamma['error'], intermediate_J1Gamma['cost'], 's:', label=' $J^\Gamma_1$', color='C3')
    ...
    axs[1, 0].plot(intermediate_J1Gamma['rot1'], intermediate_J1Gamma['rot2'], 's:', color='C3')

    # Error surface.
    error_surface = axs[1, 1].contourf(rotation_domain1, rotation_domain2, error)
    axs[1, 1].set_ylabel("Rotation domain 1")
    axs[1, 1].set_xlabel("Rotation domain 2")
    fig.colorbar(error_surface, ax=axs[1, 1], label="Error to Exact")
    axs[1, 1].plot(intermediate_J0Omega['rot1'], intermediate_J0Omega['rot2'], 's:', color='C1')
    axs[1, 1].plot(intermediate_J1Omega['rot1'], intermediate_J1Omega['rot2'], 's:', color='C2')
    axs[1, 1].plot(intermediate_J1Gamma['rot1'], intermediate_J1Gamma['rot2'], 's:', color='C3')
    ...
    error_surfaces = pd.DataFrame({'rot1': grid_rot1.flatten(), 'rot2': grid_rot2.flatten(),
                                   ...
                                   'cost1Gamma': costJ1Gamma.flatten(), 'error': error.flatten()})
